...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($bidding_price <= 0 || $quantity <= 0 || empty($location)) {
        echo "<script>alert('Invalid input. Please fill all fields correctly.');</script>";
    } else {
        $insert_query = "INSERT INTO bids (product_id, item_name, bidding_price, quantity, bidder_name, location) 
                         VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($insert_query);
        $stmt->bind_param("isdiis", $product['product_id'], $product['product_name'], $bidding_price, $quantity, $bidder_name, $location);
